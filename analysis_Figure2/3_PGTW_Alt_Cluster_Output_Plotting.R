...
# log_ICU_bed_occ <- ggplot() +
#   geom_ribbon(data = ICU_occ, aes(x = t, ymin = 0, ymax = log(ICU_occ), fill = scenario), alpha = 0.15) +
#   geom_ribbon(data = ICU_occ, aes(x = t, ymin = 0, ymax = log(ICU_occ_2), fill = scenario), alpha = 0.25) +
#   geom_ribbon(data = ICU_occ, aes(x = t, ymin = 0, ymax = log(ICU_occ_3), fill = scenario), alpha = 0.5) +
#   geom_ribbon(data = ICU_occ, aes(x = t, ymin = 0, ymax = log(ICU_occ_4), fill = scenario), alpha = 1) +
#   scale_fill_manual(values = c("#E4521B", "#8AB13C"), name = "fill") +
#   theme(legend.position = "none", axis.title = element_text(size = 10), axis.text = element_text(size = 10),
#         plot.margin = unit(c(0.5, 0, 0, 0), "cm")) +
#   scale_y_continuous(labels = comma) +
#   geom_hline(yintercept = log(actual_ICU_beds), linetype = 'dashed') +
#   geom_hline(yintercept = log(actual_MV_capacity), linetype = 'dashed') +
#   geom_hline(yintercept = log(actual_ICU_beds * actual_prop_ox_ICU_beds), linetype = 'dashed') +
#   lims(x = c(0, 320), y = c(0, 11)) +
#   labs(x = "Time (Days)", y = "ICU Bed Demand")
...
tret_prop_plot_hosp <- ggplot() +
  geom_bar(data = treat_prop_hosp, aes(x = R0, y = value, fill = interaction(R0, treat_extent), group = treat_extent),
           stat = "identity") +
...
tret_prop_plot_ICU <- ggplot() +
  geom_bar(data = treat_prop_ICU, aes(x = R0, y = value, fill = interaction(R0, treat_extent), group = treat_extent),
           stat = "identity") +
  scale_y_continuous(position = "right") +
  scale_fill_manual(values = c(alpha("#E4521B", alpha = 0.25), alpha("#8AB13C", alpha = 0.1),
                               alpha("#E4521B", alpha = 0.5), alpha("#8AB13C", alpha = 0.65),
...
# Figure 2C - Individual Level Risk With the Drug
...
partC <- plot_grid(NULL, IFRplot2, prob_ben_plot, rel_widths = c(0.6, 1.25, 1), axis = "b", align = "h", nrow = 1)
fig2 <- plot_grid(partAB, partC, ncol = 1, rel_heights = c(1.1, 2)) +
  draw_plot_label(
    c("A", "B", "C"),
    c(0, 0.48, 0),
    c(1.02, 1.02, 0.65),
    size = 30)
fig2
ggsave2("analysis_Figure2/test_Figure_2.pdf", fig2, dpi = 400, width = 12, height = 8)
