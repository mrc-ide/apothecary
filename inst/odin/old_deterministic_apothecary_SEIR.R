...
n_ICase2_Hosp_tot[] <- (n_ICase2_Hosp[i] + n_ICase2_Drug_5_Hosp[i])
...
number_req_Hosp[] <- (n_ICase2_Hosp[i] + n_ICase2_Drug_5_Hosp[i] + number_GetICU_GetOx_to_IMod[i] + number_GetICU_NoOx_to_IMod[i]) - number_req_ICU[i]  # Number of new hospitalisations that are going to require a hospital bed and oxygen (i.e. IMod)
...
number_need_ICU <- sum(number_ISev_GetICU_GetOx) + sum(number_ISev_GetICU_NoOx) + sum(number_ISev_NoICU_NoOx) +
                   sum(number_ICrit_GetICU_GetOx_GetMV) + sum(number_ICrit_GetICU_GetOx_NoMV) + sum(number_ICrit_GetICU_NoOx_NoMV) + sum(number_ICrit_NoICU_NoOx_NoMV)
...
##--------------------------------------------------------------------------------------------------------------
# Calculating Number of Individuals Requiring An ICU Bed
number_req_ICU[] <- (n_ICase2_Hosp[i] + n_ICase2_Drug_5_Hosp[i]) * prob_severe[i] # Initial number of new hospitalisations that are going to require an ICU bed (either with or w/o mechanical ventilation)
...
current_free_ICU_bed_ox <- round(current_prop_ox_ICU_beds * current_ICU_bed_capacity) + sum(n_ISev_GetICU_GetOx_Surv2_Rec) + sum(n_ISev_GetICU_GetOx_Die2_D_Hospital) +
...
current_free_ICU_bed_no_ox <- (current_ICU_bed_capacity - round(current_prop_ox_ICU_beds * current_ICU_bed_capacity)) + sum(n_ISev_GetICU_NoOx_Surv2_Rec) + sum(n_ISev_GetICU_NoOx_Die2_D_Hospital) +
...
total_GetICU_GetOx_to_IMod <- if (total_GetICU_GetOx_initial > 0 && drug_6_indic == 1) total_GetICU_GetOx_initial * (drug_6_prop_treat * drug_6_effect_size) else 0

total_GetICU_GetOx <- if(current_free_ICU_bed_ox <= 0) 0 else(if(current_free_ICU_bed_ox - (total_req_ICU - total_GetICU_GetOx_to_IMod) >= 0) (total_req_ICU - total_GetICU_GetOx_to_IMod) else(current_free_ICU_bed_ox))
number_GetICU_GetOx[] <- if (total_req_ICU <= 0) 0 else number_req_ICU[i]/sum(number_req_ICU) * total_GetICU_GetOx # sampling without replacement total_GetICU beds' worth of individuals from the N_age long vector number_req_ICU
number_GetICU_GetOx_to_IMod[] <- if (total_req_ICU <= 0) 0 else number_req_ICU[i]/sum(number_req_ICU) * total_GetICU_GetOx_to_IMod # note this only works because drug applied equally across ages. Otherwise, would need to do a different fraction for
...
number_GetICU_NoOx_to_IMod[] <- if (sum(number_req_ICU_remaining) == 0) 0 else number_req_ICU_remaining[i]/sum(number_req_ICU_remaining) * total_GetICU_NoOx_to_IMod # note this only works because drug applied equally across ages. Otherwise, would need to do a different fraction for
